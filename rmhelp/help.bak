{    Модуль Help предназначен для создания справочных служб программ,
     написанных на языке Turbo Pascal.
      Особое спасибо Б.В Михайлову за ценные советы.
...
Implementation
 Var
    ...
    ScrollLn :Integer;
    f:Text;
    Rs,BName,New1,New2,Old1,Old2:String[70];
    P:^Safe;
    V,M:Boolean;
...
End;
{-----------------------------------------------}
Procedure InitScroll(x,y,ly:Byte);
Begin
  ScrollX:=x;
  ScrollY:=y+1;
  ScrollLY:=ly-2;
  ScrollLn:=BaseEnd-OrginBase;
End;
{-----------------------------------------------}
Procedure PutScroll(BC,CC:Byte);
Begin
  for i:=0 to ScrollLY do
   begin
    Vm[ScrollY+i,ScrollX].A:=(BC Shl 4)Or CC;
    Vm[ScrollY+i,ScrollX].C:=byte('░');
   end;
    Vm[ScrollY-1,ScrollX].A:=(BC Shl 4)Or CC;
    Vm[ScrollY-1,ScrollX].C:=byte('');
    Vm[ScrollY+ScrollLY+1,ScrollX].A:=(BC Shl 4)Or CC;
    Vm[ScrollY+ScrollLY+1,ScrollX].C:=byte('');
End;
{-----------------------------------------------}
Procedure ShowScrollPip(Line:Word;BC,CC:Byte);
var OfsY:byte;
Begin
    PutScroll(3,1);
    OfsY:=Round(ScrollLy*Line/(ScrollLn-Pg));
    Vm[ScrollY+OfsY,ScrollX].A:=(BC Shl 4)Or CC;
    Vm[ScrollY+OfsY,ScrollX].C:=byte('■');
End;
{-----------------------------------------------}
...
If (Base<>OldBase)Or Not V Then
 Begin
  if (BaseEnd-OrginBase>Pg) Then ShowScrollPip(Base-OrginBase,15,1);
 FindBase(F,Base);
 DetermCount;
