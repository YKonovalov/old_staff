...
Label 1;
Type
  r=array[1..15,1..8,1..15] of string[6];
  sq=string[6];
Var
...
while c<>#13 do
Begin
   c:=readkey;if keypressed then begin c:=readkey;b:=true end;
  if (not b)and(c<>#13)and(length(st)<=2) then
...
{ else if c=#83 then ChangeChain(i)};
     if v<>j then while (p[i,j,k]='0')and(k>1) do k:=k-1;
        gotoxy(j*10-7,k+5);
       if (c>=#48)and(c<=#150) then begin SRead(sr,c);
                          if v<>0 then
                          Begin
                           p[i,j,k]:=pr(sr);
                           if (p[i,j,k]='0')and(p[i,j,k+1]<>'0') then remv;
                           if (k<>15)and(p[i,j,k]<>'0') then inc(k);
                           gotoxy(j*10-7,k+5);
                          End;
                    end;
End;
