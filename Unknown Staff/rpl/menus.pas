UNIT MENUS;
 INTERFACE
 Uses Vision,Crt,Graph,Memory,Control,Disk,Dos;
  Var FlowerNumber:Byte;
  {---------------------------------}
   Procedure Menu(Clear,r:boolean;x,y:word;Var c:Char);
   Procedure PutEditValue(v,cf:Byte);
   Procedure PutEdit;
   Procedure Vmenu(v:Byte;d:Integer);
  ...
  procedure PutRep(cf:byte);
   begin
    ...
    rectangle(x+4*sx+qx div 2,y+6*Qy+qy div 2 +3*lity,
        x+13*qx+qx div 2,y+7*qy+qy div 2+3*lity);
    Outtextxy(x+5*Sx+qx,y+7*qy+2*lity,'REPLACE');
  End;
  {==============================================}
...
Begin
  If r Then
    ...
    exit;
    End;
If x>lx-dmx-qx-10 then x:=lx-OtsX-dmx-qx-10;
If x<FieldX then x:=FieldX+1;
If y>ly-dmy-qy-10 then y:=ly-OtsY-dmy-qy-10;
if y<FieldY then y:=FieldY+5;
 Xm:=x;
 Ym:=y;
 If vm And Clear
   Then
    ...
    Vm:=False;
     ...
     for i:=1 to 4 do
     putdubs(i,0,7,0,3);
     putstrl(sx,sy,1,x+qx,y+qy+sy,7,0,3,0);
     putstrl(sx,sy,2,x+5*qx,y+qy+sy,7,0,3,0);
     putstrl(sx,sy,4,x+3*qx,y+qy,7,0,3,0);
     putstrl(sx,sy,3,x+3*qx,y+5*qy,7,0,3,0);
      puttab(sx,sy,x+qx+sx,y+qy+sy,7,0,3,0);
     puthelp(7);
     putScale(true,Scale);
   ...
   IF (k=left)And Not Vm
      Then
       ...
       If ((i>x+qx)and(j>y+qy+sy)
        And (i<x+sx+qx)and (j<y+2*sy+qy))Or (FlowerNumber=72)
       then
         Begin
          hide;
        putstrl(sx,sy,1,x+qx,y+qy+sy,10,0,11,0);
       ...
       then
         Begin
          hide;
     putstrl(sx,sy,2,x+5*qx,y+qy+sy,10,0,11,0);
     ...
     putstrl(sx,sy,2,x+5*qx,y+qy+sy,7,0,3,0);
       ...
       then
         Begin
          hide;
     putstrl(sx,sy,4,x+3*qx,y+qy,10,0,11,0);
     ...
     putstrl(sx,sy,4,x+3*qx,y+qy,7,0,3,0);
       ...
       then
         Begin
          hide;
     putstrl(sx,sy,3,x+3*qx,y+5*qy,10,0,11,0);
     ...
     putstrl(sx,sy,3,x+3*qx,y+5*qy,7,0,3,0);
       ...
       then
         Begin
          hide;
     puttab(sx,sy,x+qx+sx,y+qy+sy,12,0,2,0);
      ...
      If (i>3*qx+5)and(j>Uy-OtsY)
       ...
       then
         Begin
          hide;
         StrlPort;
        putstrl(qx,qy,1,3*qx+5,Uy-OtsY,10,0,11,1);
         WorkPort;
          If pageX-1>0 Then SetPage(-1,0);
          StrlPort;
       ...
       then
         Begin
          hide;
          StrlPort;
        putstrl(qx,qy,2,ux-Qx-1-OtsX,uy-otsY,10,0,11,0);
          WorkPort;
          If pageX<MaxPage-N+2 Then SetPage(1,0);
          StrlPort;
        putstrl(qx,qy,2,ux-Qx-1-OtsX,uy-otsY,1,2,13,10);
          WorkPort;
          show;
         End
         Else
       ...
       then
         Begin
          hide;
          StrlPort;
        putstrl(Qx,Qy,4,ux-OtsX,Qy,10,0,11,0);
          WorkPort;
          If PageY-1>0 Then SetPage(0,-1);
          StrlPort;
       ...
       then
         Begin
          hide;
          StrlPort;
        putstrl(qx,qy,3,ux-OtsX,Uy-Qy-1-otsY,10,0,11,0);
          WorkPort;
          If pageY<MaxPage-M+2 Then SetPage(0,1);
          StrlPort;
        putstrl(qx,qy,3,ux-OtsX,Uy-Qy-1-OtsY,1,2,13,10);
          WorkPort;
          show;
         End
         Else
       If (i>Ux-OtsX)and(j>Uy-OtsY)
        ...
        then
         Begin
          hide;
          StrlPort;
        Puttab(qx,qy,ux-OtsX,uy-OtsY,12,0,2,0);
          WorkPort;
          If PageX+PageY<>2 Then SetPage(-PageX+1,-PageY+1);
          StrlPort;
       ...
       then
         ...
         End;
     END;
     If (j>0)and(j<qy)And(k=Left) Then Vmenu(0,i)
                                  Else Delay(80);
End;
{---------------------------------------}
Procedure Vmenu(v:Byte;d:Integer);
...
Function FindNewPos(no:byte;l:Word):Word;
 ...
 End;
{=====================================}
 Function OpenWindow(v:Byte;l:Word):Byte;
  Type
    WinType=Record
             k1,k2,l,Mind:Byte;
  ...
  iF V=7 Then
      ...
      Repeat
       ...
       If Keypressed Then c:=readkey;
       While KeyPressed Do C:=readkey;
    If c=#75 Then Begin Dec(Param.CurrentOpt);c:='^' End;
    If c=#77 Then Begin Param.CurrentOpt:=1;c:='^' End
      Until (k<>Center)Or(c in [#75,#77])
      Exit;
       End;
   ...
   c:='.';
    X:=FindNewPos(v,l);
    If v=1 Then X:=FieldX+1;
    If v=7 Then X:=ux-Otsx-15*qx;
   WindowRec:=DataArray[v];
   setfillstyle(1,7);
   Setcolor(0);
   With WindowRec Do
    Begin
...
If Mind=1 Then
           Begin
     SetColor(0);
    Outtextxy(x+L*qx-sx,Param.Sett[v]*qy+Qy Div 3+Qy Div 2+1,'*');
           End;
     Show;
     i:=0;
If Mind<>2 Then
   ...
   if (c=#72)and(last-1>=1) then
           If Pos(':',Sw[Last-1])=0 Then n:=last-1 else
                       If Last-2>=1 Then n:=Last-2 else
                            Else
         If (c=#72)And( ((Last-1<1)And Not Qw ) Or ((Last-2<1)And Qw ) )
            Then n:=k1+k2
            Else
    ...
    If (c=#77)And(Param.CurrentOpt<7) Then
       Begin Inc(Param.CurrentOpt);c:='^';Goto 1 End
    Else
    If (c=#75)And(Param.CurrentOpt>1) Then
       Begin Dec(Param.CurrentOpt);c:='^';Goto 1 End
    Else  Goto 1;
     If last<>n Then
      Begin
       Hide;
...
If (Mind=1)And(Last=Param.Sett[v]) Then
           Begin
     SetColor(0);
    Outtextxy(x+L*qx-sx,Param.Sett[v]*qy+Qy Div 3+Qy Div 2+1,'*');
           End;
...
If (Mind=1)And(n=Param.Sett[v]) Then
           Begin
     SetColor(0);
    Outtextxy(x+L*qx-sx,Param.Sett[v]*qy+Qy Div 3+Qy Div 2+1,'*');
           End;
...
If (Mind=1) Then
           Begin
     SetColor(0);
    Outtextxy(x+L*qx-sx,n*qy+Qy Div 3+Qy Div 2+1,'*');
     Param.Sett[v]:=n;
           End;
    hide;
...
begin
mn[1]:='Layout';mn[2]:='Routing';mn[3]:='Information';
...
{-------------------------------------------}
Procedure PutEditValue(v,cf:Byte);
 Var MaxPx,MaxPy,Cx,Cy,R:Integer;
     T:Array[1..4]Of Pointtype;
 Begin
  MaxPx:=3*Qx;
  MaxPy:=3*Qy;
  cy:=FieldY+(v-1)*MaxPy-v+1;
  R:=FieldY+v*MaxPy-v+1;
   SetFillStyle(1,cf);
  If R>Ly Then Begin R:=Ly;MaxPy:=R-cy; end;
  SetViewPort(3,cy,MaxPx+3,R,True);
   SetColor(0);
   Bar(0,0,MaxPx,MaxPy);
   Rectangle(0,0,MaxPx,MaxPy);
  Cx:=MaxPx Div 2;
  Cy:=MaxPy div 2;
  R:=MaxPx Div 3;
  Case v Of
   1: Begin
       Circle(Cx,Cy,R);
       Line(Cx-MaxPx div 7,Cy,Cx-R,Cy);
       Line(Cx+MaxPx div 7,Cy,Cx+R,Cy);
       Line(Cx,cy+MaxPy Div 7,cx,Cy+R);
       Line(Cx,Cy-MaxPy Div 7,cx,Cy-R);
      End;
   2: Begin
       Circle(MaxPx Div 2,MaxPy div 2,MaxPx Div 3);
      { T[1].X:=Cx+R; T[2].X:=
       T[1].Y:=Cy;}
      End;
   3: Begin
...
4: Hide;
   ...
   begin
     if ((l-1) mod 4=0)and ((l-1) mod 12<>0) then
      ...
      inc(xt,dx);
      outtextxy(xt,yt-lity,stp[l+tur-1]);
   end;
   Lu:=tur;
   show;
   C:='@';
 ...
 Begin
  Repeat
   If keypressed then c:=Fullkey(ch);
   GetPos(i,j,k);
 ...
 while c<>#13 do
  begin
   ...
   Outtextxy(xm+dx+textwidth(st)div 2,y+qy div 2,st);
      outtextxy(xl,yl-lity,stp[tur+l-1]);
      setfillstyle(1,10);
       Bar(xt-6*textwidth('_') ,yt-qy div 2+1,
           xt+6*textwidth('_') ,yt+qy div 2{-1});
           setcolor(12);
      outtextxy(xt,yt-lity,stp[Tur+n-1]);
   show;
      xl:=xt;
      yl:=yt;
      i:=0;
